...
public class DnDTabPaneSkin extends TabPaneSkin implements DragSetup {
	...
	void tabPane_handleDragOver(Pane tabHeaderArea, Pane headersRegion, DragEvent event) {
		...
		Boolean tabOrderReversed = draggedTab.getTabPane().sideProperty().get() == Side.LEFT;
		List<Node> children = Arrays.asList(headersRegion.getChildren().stream().toArray(Node[]::new));
		...
		for (Node n : children) {
			...
			if (b.getMaxX() > x) {
				if (b.getMinX() + b.getWidth() / 2 > x) {
					referenceNode = n;
					type = (tabOrderReversed)? DropType.AFTER : DropType.BEFORE;
				} else {
					referenceNode = n;
					type = (tabOrderReversed) ? DropType.BEFORE : DropType.AFTER;
				}
				break;
			}
		}
