...
import com.jdngray77.htmldesigner.backend.LoggingKt;
...
//	}
	...
	private static MarkerFeedback handleMarker(Pane layoutNode, FeedbackData data) {
		...
		double ratio = data.bounds.getWidth() / w;
//		ratio += 0.1;
		...
		if (data.draggedTab.getTabPane().sideProperty().get() == Side.LEFT) {
			marker.setRotate(180);
			wDiff = w;
		}

		if (data.dropType == DropType.AFTER) {
			LoggingKt.logStatus("AFTER : " + data.bounds);
			marker.relocate(data.bounds.getMinX() + wDiff, data.bounds.getMinY() + data.bounds.getHeight() - hDiff);
		} else {
			LoggingKt.logStatus("BEFORE : " + data.bounds);
